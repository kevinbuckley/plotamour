...
import { X, Plus, Trash2, ChevronDown, ChevronUp, Link2, StickyNote, ExternalLink } from "lucide-react";
...
import type { Character, CharacterRelation, Tag, ResearchLink } from "@/lib/types/database";
...
export function CharacterDetail({
  ...
  const [researchLinks, setResearchLinks] = useState<ResearchLink[]>(
    (character.research_links as ResearchLink[]) ?? []
  );
  ...
  useEffect(() => {
    ...
    setResearchLinks((character.research_links as ResearchLink[]) ?? []);
  ...
  const handleAddResearchLink = async () => {
    ...
    try {
      ...
      await onUpdate(character.id, { research_links: updated as any });
