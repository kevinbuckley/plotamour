...
export function TimelineGrid({
  ...
  const getScenesForCell = useCallback(
    (chapterId: string, plotlineId: string) =>
      scenes.filter((s) => s.chapter_id === chapterId && s.plotline_id === plotlineId),
    [scenes]
  );
  ...
  const handleDeleteChapter = async (id: string) => {
    ...
    if (scenesInChapter.length > 0) {
      if (!confirm(`This chapter has ${scenesInChapter.length} scene(s). Delete anyway?`)) return;
    }
  ...
  const handleDeletePlotline = async (id: string) => {
    ...
    if (scenesInPlotline.length > 0) {
      if (!confirm(`This plotline has ${scenesInPlotline.length} scene(s). Delete anyway?`)) return;
    }
              ...
              >
                ...
                {editingPlotline === plotline.id ? (
                  <input
                    ...
                    autoFocus
                    onBlur={(e) => {
                      timelineAction({ action: "updatePlotline", id: plotline.id, title: e.target.value });
                      setPlotlines((prev) => prev.map((p) => p.id === plotline.id ? { ...p, title: e.target.value } : p));
                      setEditingPlotline(null);
                    }}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        timelineAction({ action: "updatePlotline", id: plotline.id, title: e.currentTarget.value });
                        setPlotlines((prev) => prev.map((p) => p.id === plotline.id ? { ...p, title: e.currentTarget.value } : p));
                        setEditingPlotline(null);
                      }
                  ...
                  <>
                    <span
                      className="cursor-pointer text-sm font-medium"
                      onDoubleClick={() => setEditingPlotline(plotline.id)}
                    >
                      {plotline.title}
                    </span>
                    <button
              ...
              {chapters.map((chapter) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "group/cell flex min-h-[100px] flex-col gap-1 rounded-md border border-dashed border-transparent p-1.5 transition-colors",
                      cellScenes.length === 0 && "hover:border-border hover:bg-muted/30"
                    )}
                  >
